...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5514998435607@s.whatsapp.net"] // replace this with your number
			...
			const premium = ["5514998435607@s.whatsapp.net","558296156440@s.whatsapp.net"] 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("ZEUS")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/ZEUSf.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS ZEUS BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @ZEUS domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o ZEUS ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
